...
**공개 API만으로 "접속은 활발하지만 성장이 멈춘 고레벨 유저"를 찾아낸 게임 데이터 분석 프로젝트입니다.** 내부 보스·거래 로그가 없는 상황에서 캐릭터 성장 신호를 proxy로 설계하고, 운영팀이 검토할 수 있는 후보군과 설명 가능한 선별 규칙을 만들었습니다.

## 핵심 결과

- 레벨 270~290 본캐 **2,000명**의 12개월 월별 성장 스냅샷 수집
- 전투력·HEXA·유니온·어센틱 심볼의 최근 6개월 변화량으로 **주차 후보군 380명(19.0%)** 식별
- K-Means 후보군 380명 중 **355명**이 DBSCAN 밀도 군집과 정합
- 후보 비율은 레벨 구간별로 유의하게 달랐고, 직업 계열별 차이는 유의하지 않음
- RandomForest 중요도 기반 단순 규칙으로 후보 라벨을 근사: **5-fold ROC-AUC 0.850**, 운영 규칙 **FPR 0.068**

> 이 결과는 제재 모델이 아니라 **운영 검토용 후보 탐색**입니다. 공개 API에는 보스 수익·메소 거래·제재 라벨이 없어서 실제 주차 여부는 내부 로그로 추가 검증해야 합니다.

## Portfolio Summary

### 문제 정의

장기 서비스 MMORPG에서는 캐릭터 성장을 특정 구간에 멈춘 채 정기 보상만 반복 수급하는 유저가 경제 밸런스 리스크가 될 수 있습니다. 문제는 이들이 일반 고활성 유저와 외관상 비슷해 단순 규칙으로 선별하기 어렵다는 점입니다. 이 프로젝트는 **내부 로그 없이 공개 API 신호만으로 이런 유저군을 통계적으로 좁힐 수 있는지**를 검증합니다.
