...
→ 가장 방어 가능한 단일 사전 프록시는 **triple-zero ≈ 13%**. 구현 후 H3 1단계 Rule 학습에는
H1 `is_stagnant_cluster`를 사용하고, 최종 후보 Rule 평가는 반복 접속 게이트를 결합해 별도로 수행한다.
...
print(f'  3) h3_rule/h3_rule.ipynb: is_stagnant_cluster → RF/XGB → 성장 정체 Rule + 접속 게이트')
```
    ...
    [다음 단계]
      ...
      3) h3_rule/h3_rule.ipynb: is_stagnant_cluster → RF/XGB → 성장 정체 Rule + 접속 게이트
...
- **1단계 양성 표본**: H1 `is_stagnant_cluster` 394명(20.0%) → 5-fold CV per-fold 약 79명. `class_weight="balanced"` 또는 `scale_pos_weight`를 적용한다.
- **Rule 핵심 신호**: H1 직접 사용 피처(cumEXP·union delta·hexa_frag delta)와 동일 신호 파생값은 제외한다. H1 미사용 성장·상태 피처로 성장 정체 Rule을 근사하고, 최근 분기 반복 접속 게이트를 별도로 결합한다.
...
- **H3 제외 `hexa_fragments` 계열**: H1에서 `avg_monthly_delta_hexa_frag`를 채택했으므로 `hexa_fragments_total`과 recent 파생값도 H3 입력에서 제외한다.
